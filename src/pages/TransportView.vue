  ...
  <div class="transport-page">
    ...
    <div class="search-wrap">
      <input
        ...
        class="search-input"
        type="search"
        placeholder="Søk etter fremkomstmiddel…"
        autocomplete="off"
        autocorrect="off"
        spellcheck="false"
      />
    ...
    >
      <div v-if="loading" class="status">Laster fremkomstmidler…</div>
        ...
        <div :style="{ transform: `translateY(${offsetY}px)` }">
          ...
          >
            <img
              ...
              alt=""
            />
            <span v-else class="transport-thumb-placeholder"></span>
            <span class="transport-name">{{ item.name }}</span>
            <span v-if="item.type" class="transport-type">{{ item.type }}</span>
          </RouterLink>
        </div>
      </div>
    </div>
    ...
    <div class="count-bar">
      <span v-if="query.length >= 2">
        Viser {{ fmt(filtered.length) }} av {{ fmt(transport.length) }} fremkomstmidler
      </span>
      <span v-else>Viser {{ fmt(transport.length) }} fremkomstmidler</span>
    </div>
  </div>
</template>
...
const filtered = computed(() => {
  if (query.value.length < 2) return transport.value
...
.transport-row:hover { background: var(--color-bg); }
