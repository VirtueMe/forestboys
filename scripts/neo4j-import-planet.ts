...
// Slug → sanityId lookup for converting resolved slugs to Neo4j primary keys
const personSanityIdBySlug = new Map<string, string>()
for (const p of rawPeople) {
  if (p.slug?.current && p._id) personSanityIdBySlug.set(p.slug.current, p._id)
}

// Event slug → sanityId lookup
const eventSanityIdBySlug = new Map<string, string>()
for (const ev of rawEvents) {
  if (ev.slug?.current && ev._id) eventSanityIdBySlug.set(ev.slug.current, ev._id)
}
  ...
  try {
    ...
    for (const ev of planetEvents) {
      ...
      try {
        ...
        const evSanityId = eventSanityIdBySlug.get(ev.slug) ?? ev._id
        await tx.run(
          `MATCH (e:Event {sanityId: $evId}), (o:Operation {id: $opId})
           MERGE (e)-[:DESCRIBES]->(o)`,
          { evId: evSanityId, opId: opNodeId },
        )
        ...
        for (const entry of roster) {
          ...
          // Resolve to sanityId: auto-stubs use their own slug as sanityId
          const isAutoStub = method === 'auto-generated'
          const personId   = isAutoStub
            ? personSlug
            : (personSanityIdBySlug.get(personSlug) ?? personSlug)

          // If auto-generated: ensure the stub Person node exists (sanityId = slug)
          if (isAutoStub) {
            await tx.run(
              `MERGE (p:Person {sanityId: $id})
               ON CREATE SET p.slug          = $id,
                             p.name          = $name,
                             p.autoGenerated = true,
                             p.source        = "planet-roster"`,
              { id: personId, name: entry.rawName },
            )
          }

          // If this entry has a real override slug, retire the old auto stub
          if (method === 'override' && personSlug) {
            const staleId = autoSlug(entry.rawName)
            await tx.run(
              `OPTIONAL MATCH (stub:Person {sanityId: $staleId})
          ...
          await tx.run(
            `MATCH (p:Person {sanityId: $id}), (o:Operation {id: $opId})
          ...
          if (entry.aahlyDate) {
            ...
            await tx.run(
              `MATCH (p:Person {sanityId: $id}), (b:TrainingBatch {id: $bId})
               MERGE (p)-[r:TRAINED_IN]->(b)
               SET r.arrived_sweden = $arrivedSweden`,
              { id: personId, bId, arrivedSweden: entry.arrivedSweden ?? null },
            )
          }
        }
